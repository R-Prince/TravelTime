...
### Modal Page: Google map and place API search results
Once the user selects a city from the autocomplete list a modal page will appear with the results from the search. The modal will first display the name of the city and the country in which the city is in. Secondly using the google maps api a map is displayed with markers showing “Points of Interest” which includes tourist attractions in the area. The user can then click on the marker which displays a small info-window which displays information about the attraction/restaurant such as name, address and a rating between 1 and 5 based on reviews from customers. Scrolling down the modal page using the google place api three attractions and six hotels within the searched city area is then displayed using cards. The card displays an image of the establishment and information such as name, address and rating out of five displayed using stars. At the bottom of the card there is also a call to action button which prompts the user to “Visit the website” which opens a new tab and directs them to a website to book the hotel/attraction. The bottom and top of the modal page also includes buttons which allows the user to close the modal page. The recommendation section has slightly more information and is added to the modal page which includes an image of the city and a “Quick guide” which gives the user more information about the city such as the language spoken, currency used, the nearest airport and a short paragraph describing the beauty of the city. The images and the quick guide is used as we want to make sure the user has enough information to go ahead and book accommodation with one of the sponsors. 
...
Pop-up modal page - this allows the user to view information on the city they have selected. Using a modal page allows the user to clearly and easily see information on a city. There are two different modal pages that are used one for the search results and one page for the Travel Time pick results. 
